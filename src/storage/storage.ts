 ...
 *
 * 원칙
 * - 화면(app/)에서 AsyncStorage 를 직접 부르지 않는다. 항상 이 파일을 거친다.
 * - 읽기/파싱에 실패해도 앱을 죽이지 않는다. 기본값을 돌려준다.
 * - ⚠️ 저장 값(설문 응답·통증 기록)을 console 에 출력하지 않는다.
...
/**
 * 사용자 데이터 전체 삭제.
 * 설정 > '내 데이터 전체 삭제' 에서만 호출한다. 되돌릴 수 없다.
 */
