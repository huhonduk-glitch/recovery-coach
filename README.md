...
앱 안에서 **내정보 → 운동·프로그램 편집** 으로 들어가면 동작 설명·코칭 문구·세트·횟수와
프로그램 편성까지 직접 고칠 수 있습니다. 고친 내용은 앱 전체(운동 상세·루틴 실행 포함)에
바로 반영됩니다.

안전을 위해 아래는 저장이 막힙니다.

- 운동의 **주의사항**, 프로그램의 **중단 기준·주의사항** 을 비우는 것
- **통증을 참으라는 표현** (`아파도 계속`, `버티세요`, `한계까지` 등)
- 목록에 없는 운동을 가리키는 프로그램

`처방`·`진단` 같은 의료 표현은 막지 않고 경고만 합니다. '물리치료사와 상의하세요' 처럼
정당하게 쓰는 경우가 있기 때문입니다.

기본 운동은 **지우지 않고 감춥니다.** 언제든 되돌릴 수 있습니다.
고친 내용은 이 기기에만 저장되므로, 오래 남기려면 **코드로 내보내기** 로 뽑아
`src/data/` 에 붙여 넣으세요. 자세한 규칙은 [SAFETY_POLICY §23](docs/SAFETY_POLICY.md).
